...
export const ProceduralGroundBackground: React.FC = () => {
    ...
    `;
        ...
        const render = (time: number) => {
            // Dynamic scaling without forcing dom paints unless necessary
            const { innerWidth: width, innerHeight: height } = window;
            if (canvas.width !== width || canvas.height !== height) {
                canvas.width = width;
                canvas.height = height;
    ...
    return (
        <div className="fixed inset-0 w-full h-full bg-background -z-50 pointer-events-none">
            <canvas
                ref={canvasRef}
                className="w-full h-full block touch-none"
                style={{ filter: 'contrast(1.1) brightness(0.9)', opacity: 0.6 }}
            />
        </div>
    );
};
